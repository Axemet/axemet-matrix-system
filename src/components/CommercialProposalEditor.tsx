        ...
        {items.map((item, index) => {
          ...
          return <div key={item.id} className="border-t border-slate-100 first:border-t-0">
            <div className="grid gap-3 px-4 py-4 md:grid-cols-[minmax(0,1fr)_90px_150px_84px] md:items-center">
              ...
              <div className="flex justify-end gap-1"><button type="button" onClick={() => onEditTechnicalItem(item.id)} className="inline-flex items-center gap-1 rounded-lg border border-indigo-200 bg-indigo-50 px-2 py-1.5 text-xs font-bold text-indigo-800 hover:bg-indigo-100" title="Reabrir todos os campos técnicos deste item"><Calculator className="h-3.5 w-3.5" />Orçamento</button><button type="button" onClick={() => setEditingId(isEditing ? null : item.id)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2 py-1.5 text-xs font-bold text-slate-700 hover:border-indigo-200 hover:bg-indigo-50" title="Editar dados comerciais"><Pencil className="h-3.5 w-3.5" />{isEditing ? 'Fechar' : 'Editar'}</button><button type="button" onClick={() => onItemsChange(items.filter(current => current.id !== item.id))} className="rounded-lg p-1.5 text-rose-600 hover:bg-rose-50" title="Remover da proposta"><Trash2 className="h-4 w-4" /></button></div>
            </div>
