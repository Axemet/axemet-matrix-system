...
export default function CommercialProposalEditor({ items, terms, onItemsChange, onTermsChange, onAddCurrentTechnicalItem, onPrepareNextTechnicalItem }: Props) {
  ...
  const updateItem = (id: string, patch: Partial<ProposalItem>) => onItemsChange(items.map(item => item.id === id ? { ...item, ...patch } : item));
    ...
    <section className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-200">
        <table className="w-full min-w-[640px] text-left text-xs">
          ...
          <tbody>{items.length ? items.map((item, index) => <tr key={item.id} className="border-t border-slate-100"><td className="px-3 py-3"><p className="mb-1 text-[10px] font-black uppercase tracking-wider text-slate-400">Item {String(index + 1).padStart(2, '0')} · dados de apresentação ao cliente</p><div className="grid gap-2 sm:grid-cols-2"><label className="grid gap-1 text-[9px] font-black uppercase tracking-wider text-slate-500">Título do ferramental<input value={item.title || ''} onChange={e => updateItem(item.id, { title: e.target.value })} placeholder="Ex.: Conjunto Delta Frio" className="rounded-md border border-slate-200 px-2 py-1.5 text-xs font-semibold normal-case tracking-normal text-slate-800 outline-none focus:border-orange-400" /></label><label className="grid gap-1 text-[9px] font-black uppercase tracking-wider text-slate-500">Tipo de molde<input value={item.description} onChange={e => updateItem(item.id, { description: e.target.value })} placeholder="Ex.: Molde de injeção" className="rounded-md border border-slate-200 px-2 py-1.5 text-xs font-semibold normal-case tracking-normal text-slate-800 outline-none focus:border-orange-400" /></label></div><p className="mt-1.5 text-[10px] text-slate-400">Ref. técnica: {item.sourceTechnicalReference || 'cálculo técnico do orçamento'}</p></td><td className="px-3 py-3 text-center font-semibold text-slate-700">{item.quantity}<span className="mt-1 block text-[9px] font-medium text-slate-400">conjunto</span></td><td className="px-3 py-3 text-right font-black text-slate-900">{formatCurrency(item.quantity * item.unitPrice)}<span className="mt-1 block text-[9px] font-medium text-slate-400">valor técnico bloqueado</span></td><td className="px-2 py-3"><button type="button" onClick={() => onItemsChange(items.filter(current => current.id !== item.id))} className="rounded-md p-1.5 text-rose-600 hover:bg-rose-50" title="Remover da proposta"><Trash2 className="h-4 w-4" /></button></td></tr>) : <tr><td colSpan={4} className="px-3 py-8 text-center text-xs text-slate-400">Nenhum item técnico foi incorporado. Calcule o primeiro item e clique em “Incorporar cálculo atual”.</td></tr>}</tbody>
        </table>
      </div>
