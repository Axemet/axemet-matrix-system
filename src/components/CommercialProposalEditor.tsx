...
interface Props {
  ...
  onEditTechnicalItem: (id: string) => void;
...
export default function CommercialProposalEditor({ items, terms, onItemsChange, onTermsChange, onEditTechnicalItem, onAddCurrentTechnicalItem, onPrepareNextTechnicalItem }: Props) {
  ...
  return (
    <section className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      ...
      <div className="overflow-hidden rounded-xl border border-slate-200">
        <div className="hidden grid-cols-[minmax(0,1fr)_72px_140px_218px] bg-slate-50 px-4 py-2 text-[10px] font-black uppercase tracking-wider text-slate-500 lg:grid"><span>Item técnico consolidado</span><span className="text-center">Qtd.</span><span className="text-right">Valor consolidado</span><span className="text-right">Ações</span></div>
        ...
        {items.map((item, index) => {
          ...
          return <div key={item.id} className="border-t border-slate-100 first:border-t-0">
            <div className="grid gap-3 px-4 py-4 lg:grid-cols-[minmax(0,1fr)_72px_140px_218px] lg:items-center">
              ...
              <div className="flex min-w-[210px] justify-end gap-1"><button type="button" onClick={() => onEditTechnicalItem(item.id)} className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-indigo-200 bg-indigo-50 px-2 py-1.5 text-xs font-bold text-indigo-800 hover:bg-indigo-100" title="Reabrir todos os campos técnicos deste item"><Calculator className="h-3.5 w-3.5" />Orçamento</button><button type="button" onClick={() => setEditingId(isEditing ? null : item.id)} className="inline-flex shrink-0 items-center gap-1 rounded-lg border border-slate-200 px-2 py-1.5 text-xs font-bold text-slate-700 hover:border-indigo-200 hover:bg-indigo-50" title="Editar dados comerciais"><Pencil className="h-3.5 w-3.5" />{isEditing ? 'Fechar' : 'Editar'}</button><button type="button" onClick={() => onItemsChange(items.filter(current => current.id !== item.id))} className="shrink-0 rounded-lg p-1.5 text-rose-600 hover:bg-rose-50" title="Remover da proposta"><Trash2 className="h-4 w-4" /></button></div>
            </div>
